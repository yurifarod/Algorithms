...
class_file = './dataset/housing_in_london.csv'
df = pd.read_csv(class_file, index_col=False, sep=',')
df.drop('area', inplace=True, axis=1)
df.drop('code', inplace=True, axis=1)
df.drop('average_price', inplace=True, axis=1)
df.drop('no_of_crimes', inplace=True, axis=1)
df.drop('borough_flag', inplace=True, axis=1)
...
#Esta serie tem as duas ultimas observacoes vazias, precisaremos tratar isso!

serie_original = serie_original.drop('2019-12-01')
serie_original = serie_original.drop('2020-01-01')
...
print('A serie parece ter menos ruidos na primeira diferenciação! Manteremos D = 2')

from statsmodels.graphics.tsaplots import plot_pacf
plot_pacf(serie_original.diff().dropna())

print('A serie tem pico de autocorrelação em P = 2')

plot_acf(serie_original.diff().dropna())

print('A serie tem picos fora de faixa em Q = 12')
